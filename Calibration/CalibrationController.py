...
class CalibrationController:
    ...
    def run(self):
        ...
        try:
            while self._running:
                ...
                self._handle_events()
                self._update()
                self._view.render() # View now pulls from model itself
    ...
    def _handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self._model.phase == CalibrationPhase.EXPLANATION:
                    if self._view.is_start_clicked(event.pos):
    ...
    def _calculate_results(self):
        ...
        alpha, beta = self._signal_processor.process_calibration_data(
            self._model.relaxed_data, 
            self._model.concentrated_data
        )
        self._model.alpha_ratio = alpha
        self._model.beta_ratio = beta
        print(f"Calculated Thresholds -> Alpha: {alpha:.2f}, Beta: {beta:.2f}")
