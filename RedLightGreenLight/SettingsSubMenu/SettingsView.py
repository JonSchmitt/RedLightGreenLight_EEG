...
class SettingsView:
    ...
    def __init__(self, settings_model:SettingsModel, screen:pygame.Surface):
        ...
        self._vbox = VBox(pygame.Rect(0, 0, vBox_width, vBox_height), self._manager, spacing=10,
                    anchors={'centerx': 'centerx', 'centery': 'centery'})

        # create buttons
        button_width = int(200 * self._settings.get_ui_scaling())
        button_height = int(50 * self._settings.get_ui_scaling())
        self._ok_button = pygame_gui.elements.UIButton(pygame.Rect(0, 0, button_width, button_height), text="OK",manager=self._manager, container=self._vbox.get_panel())

        # create Checkboxes
        checkbox_width = int(50 * self._settings.get_ui_scaling())
        checkbox_height = checkbox_width
        self._music_checkbox = pygame_gui.elements.UICheckBox(pygame.Rect(0, 0, checkbox_width, checkbox_height), text="Music",manager=self._manager, container=self._vbox.get_panel())
        ...
        self._warning_time_slider = SliderWithLabel(manager=self._manager, container=self._vbox.get_panel(), width=slider_width, height=slider_height, label_text="Warning time", start_value=self._settings.get_warning_time(), value_range=(0, 25), suffix="s")
        self._switch_time_slider = SliderWithLabel(manager=self._manager, container=self._vbox.get_panel(), width=slider_width, height=slider_height, label_text="Switch time", start_value=self._settings.get_switch_time(), value_range=(0, 25), suffix="s")
    ...
    def show(self,time_delta)->None:
        ...
        self._music_checkbox.show()
        self._warning_time_slider.show()
        self._switch_time_slider.show()
    ...
    def hide(self)->None:
        ...
        self._music_checkbox.hide()
        self._warning_time_slider.hide()
        self._switch_time_slider.hide()

        self._manager.update(0.0)
        self._manager.draw_ui(self._screen)
        pygame.display.flip()
